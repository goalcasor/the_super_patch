...
import { questions_, Question } from '@/data/QuestionsLandingTest';
...
export default function LandingTest() {
  ...
  const handleAnswerSelection = (index: number, answer: string): void => {
    ...
    if (swiperRef.current && index < questions_.length - 1) {
      swiperRef.current.slideNext();
    } else if (index === questions_.length - 1 && swiperRef.current) {
      swiperRef.current.slideNext();
  ...
  const evaluateResponses = (responses: string[]): void => {
    ...
    responses.forEach((response: string) => {
      if (response === "a) Sí" || response === "a) Más tiempo" || response === "a) Más dinero" || response === "a) Me gustaría saber más") {
        score += 2; // Respuesta "a" suma 2 puntos.
      }
      if (response === "c) Estoy en camino" || response === "c) No lo sé" || response === "c) Me gustaría saber más") {
        score += 1; // Respuesta "c" suma 1 punto.
      }
      // No se suma puntaje para respuestas "b)" (No).
    });
  
    const isQualified: boolean = score >= 8; // Ajusta el umbral de puntuación según el total de preguntas.
  ...
  return (
    <div className={styles.landing_test_page}>
      ...
      >
        {questions_.map((item: Question, index: number) => (
            ...
            {evaluation && (
              <div className={styles.evaluation_container}>
                <div className={styles.evaluation_summary}>
                  ...
                  <p className={styles.evaluation_score}>Preguntas totales: {questions_.length}</p>
